...
/**
 * Класс сети Джордана-Элмана.
 * Реализует обучение и предсказание временных рядов.
 */
public class ElmanJordanNetwork {
    ...
    public ElmanJordanNetwork(List<Double> seq, int inputSize, int hiddenSize,
        ...
        // Ожидаемые значения (сдвинуты на размер окна входа)
    ...
    public int train() {
        ...
        while (iter < maxIters && error > maxError) {
            // Накопители градиентов для эпохи
            ...
            // Вектор предыдущих выходов (эффектор)
            ...
            for (int i = 0; i < expected.size(); i++) {
                ...
                // h_input = input * W + context * W_C + prev_outputs * W_O
                // (1xIn)*(InxHid) + (1xCtx)*(CtxxHid) + (1xEff)*(EffxHid) -> (1xHid)
            ...
            // --- Обратный проход (Backpropagation Through Time) ---
            Matrix dHiddenNext = Matrix.zero(1, hiddenSize);

            for (int i = expected.size() - 1; i >= 0; i--) {
                double diff = outputs.get(i) - expected.get(i);

                // dW_ += diff * hidden_states[i]^T -> (Hidx1)
                // hiddenStates[i] это (1xHid), транспонируем -> (Hidx1)
                dW_ = dW_.add(hiddenStates.get(i).transpose().multiplyScalar(diff));

                // dOutputHidden = diff * W_^T -> (1x1) * (1xHid) = (1xHid)
                ...
                // dW += inputs[i]^T * dHInput -> (Inx1) * (1xHid) = (InxHid)
    ...
    public List<Double> predict() {
        ...
        // Подготовка контекста на последних данных (warm-up)
        context.setZero();
        Matrix prevOutputs = Matrix.zero(1, effectorSize);

        // "Разогрев" контекста на обучающей выборке
        // Прогон по всей обучающей выборке для установки контекста
        for (int i = 0; i < expected.size(); i++) {
            double[] inpArr = new double[inputSize];
            for (int j = 0; j < inputSize; j++) inpArr[j] = sequence.get(i + j);
            Matrix input = Matrix.fromVector(inpArr, true);
        ...
        // Берем последний доступный вход
